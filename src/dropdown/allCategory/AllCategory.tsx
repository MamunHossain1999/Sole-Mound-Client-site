...
const parentCategories: ParentCategory[] = [
  {
    name: "All Categorys",
    sub: [
      { name: "All Category", link: "/category-search-page" },
      {
  ...
  },
  {
    ...
    sub: [
      ...
      },
      {
        ...
        link: "/category/smart-home/smart-home-lighting",
      },
      {
  ...
  },
  {
    ...
    sub: [
      ...
      { name: "Building Toys", link: "/category/toys-and-games/building-toys" },
      {
      ...
      { name: "Grown-Up Toys", link: "/category/toys-and-games/grown-up-toys" },
...
const AllCategory: React.FC = () => {
  return (
    <div className="relative inline-block text-left group">
      <button
        type="button"
        className="inline-flex justify-center w-full py-2 px-4 text-base items-center gap-1.5 font-semibold text-[#1F1F1F] bg-white border border-gray-200 rounded-md cursor-pointer hover:bg-gray-50 transition-colors"
      >
        <svg
        ...
        All Categories
        <svg
          className="ml-1 h-5 w-5 text-gray-500 transition-transform group-hover:rotate-180"
          fill="none"
          stroke="currentColor"
          viewBox="0 0 24 24"
        >
      ...
      {/* Main Dropdown */}
      <div className="absolute left-0 mt-2 w-64 bg-white rounded-xl shadow-xl opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-200 z-50 py-2 border border-gray-100">
        {parentCategories.map((parent) => (
          <div key={parent.name} className="relative group/submenu">
            {/* Parent Category - এখন শুধু টেক্সট, কোনো Link নয় */}
            <div className="flex justify-between items-center px-4 py-3 text-base font-medium text-[#1F1F1F] hover:bg-gray-50 transition-colors cursor-default">
              {parent.name}
              {parent.sub && parent.sub.length > 0 && (
                <svg
                  className="h-4 w-4 text-gray-400 transition-transform group-hover/submenu:rotate-90"
                  fill="none"
                  stroke="currentColor"
                  viewBox="0 0 24 24"
                >
            ...
            {/* Submenu Dropdown */}
            {parent.sub && parent.sub.length > 0 && (
              <div className="absolute left-full top-0 ml-1 w-72 bg-white rounded-xl shadow-xl opacity-0 invisible group-hover/submenu:opacity-100 group-hover/submenu:visible transition-all duration-200 z-50 border border-gray-100">
                <div className="py-2 max-h-[450px] overflow-y-auto custom-scrollbar">
                  {parent.sub.map((sub) => (
                    <Link
                      key={sub.name}
                      to={sub.link}
                      className="block px-5 py-2.5 text-[15px] text-gray-700 hover:bg-gray-50 hover:text-[#4B0081] transition-colors whitespace-nowrap"
                    >
                      {sub.name}
                    </Link>
                  ))}

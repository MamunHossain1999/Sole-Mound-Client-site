        ...
        {data.map((product, index) => (
          <SwiperSlide key={index}>
            <div className="relative group bg-[#F8F8F8] hover:bg-[#FDF1F7] rounded-lg overflow-hidden  cursor-pointer duration-400 hover:border hover:border-[#C8A8E9]">
              ...
              {/* 👇 Hover Icons Group - centered on image */}
              <div className="absolute inset-0 flex items-center justify-center bg-opacity-10 opacity-0 group-hover:opacity-100 transition-opacity duration-300 z-10">
                <div className="flex gap-4">
                  <button className="bg-white p-4 rounded-full cursor-pointer hover:bg-[#C8A8E9]">
              ...
              <div className="p-6">
                ...
                <h3 className="font-medium text-[#1F1F1F] text-[16px] mb-2 line-clamp-2 h-12">{product.name}</h3>

                <div className="flex items-center mb-2">
                  <span className="text-[#3CA6FC] text-lg font-bold ">${product.price}</span>
                </div>
              </div>
            </div>
          </SwiperSlide>

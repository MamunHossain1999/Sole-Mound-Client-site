...
const DashBoard: React.FC = () => {
  ...
  const { data: user, isLoading } = useGetProfileQuery(); // RTK query
  const [logoutUser] = useLogoutUserMutation(); // RTK mutation
  ...
  const getNavLinkClass = (isActive: boolean): string => {
    const baseClass =
      "group flex gap-3 items-center transition-all duration-200 hover:text-[#A8537B]";
    return isActive
  ...
  const iconClass =
    "w-6 h-6 transition-all duration-200 group-hover:brightness-0 group-hover:contrast-0 group-hover:sepia group-hover:hue-rotate-[320deg] group-hover:saturate-200 group-hover:brightness-75";
  const activeIconClass =
    "group-[.active-nav]:brightness-0 group-[.active-nav]:contrast-0 group-[.active-nav]:sepia group-[.active-nav]:hue-rotate-[320deg] group-[.active-nav]:saturate-200 group-[.active-nav]:brightness-75";
    ...
    <div className="w-full min-h-screen flex flex-col bg-white border">
      ...
      <div className="flex flex-col md:flex-row flex-1 container mx-auto md:px-4 px-2 py-6 md:gap-0 lg:gap-6">
        ...
        <aside className="w-full md:w-52 lg:w-64 top-40 lg:top-40 h-fit px-2 py-9">
          <div className="flex items-center gap-3 text-base text-[#919191] px-2 pb-6">
            {/* Avatar */}
            <img
              src={
                user?.avatar || "https://i.ibb.co/2nG5z4S/default-avatar.png"
              }
              alt="user avatar"
            ...
            {/* Text */}
            <div>
              <p className="text-sm text-[#919191]">Welcome,</p>
